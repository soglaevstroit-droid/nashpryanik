...
На текущем этапе здесь нет бизнес-модулей, tasks, photos, coins или AI. Модуль `events` сохраняет события утвержденных типов. Модуль `processes` хранит только текущее состояние процесса; историю изменений хранит Event Engine. Модуль `auth` реализует только фундамент входа, JWT и RBAC.
