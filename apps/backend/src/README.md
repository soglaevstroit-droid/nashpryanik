...
На текущем этапе здесь нет photos, coins или AI. Модуль `events` сохраняет события утвержденных типов. Модуль `processes` хранит только текущее состояние процесса; историю изменений хранит Event Engine. Модуль `auth` реализует только фундамент входа, JWT и RBAC. Модуль `work-shifts` фиксирует начало и завершение смены текущего пользователя. Модуль `tasks` хранит текущее состояние задачи, а историю изменений передает в Event Engine. Модуль `task-steps` хранит текущие шаги выполнения задачи, а события шагов передает в Event Engine.
